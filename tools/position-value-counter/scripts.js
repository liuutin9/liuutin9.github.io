...
function calculatePositionValue() {
    ...
    // 計算公式：position_value = tuition_fee / (abs((cross_price - current_price) / current_price) + trading_fee_rate)
    const priceDiffRatio = Math.abs((crossPrice - currentPrice) / currentPrice);
    const positionValue = tuitionFee / (priceDiffRatio + tradingFeeRate);

    // 格式化輸出結果
    resultDisplay.style.borderLeftColor = "var(--bird-orange)";
    resultDisplay.innerHTML = `建議倉位價值 (Position Value)：<br><span class="result-highlight">${positionValue.toFixed(4)}</span>`;
}